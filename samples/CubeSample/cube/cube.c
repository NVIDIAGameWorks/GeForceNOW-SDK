...
#define APP_SHORT_NAME "CubeSample"
#define APP_LONG_NAME "Cube Sample"
...
#define ERR_EXIT(err_msg, err_class)                                           \
    do {                                                                       \
        ((void)__android_log_print(ANDROID_LOG_INFO, "Cube Sample", err_msg)); \
        exit(1);                                                               \
    } while (0)
...
void DbgMsg(const char *fmt, ...) {
    va_list va;
    va_start(va, fmt);
    __android_log_print(ANDROID_LOG_INFO, "Cube Sample", fmt, va);
    va_end(va);
}
#else  // VARARGS_WORKS_ON_ANDROID
#define DbgMsg(fmt, ...)                                                                  \
    do {                                                                                  \
        ((void)__android_log_print(ANDROID_LOG_INFO, "Cube Sample", fmt, ##__VA_ARGS__)); \
    } while (0)
...
static char* tex_files[] = { "nvidia.ppm" };
...
VKAPI_ATTR VkBool32 VKAPI_CALL debug_messenger_callback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
    ...
    if (pCallbackData->objectCount > 0) {
        ...
        for (uint32_t object = 0; object < pCallbackData->objectCount; ++object) {
            // Suppress CppCheck warnings in original OSS vkcube code
...
static void demo_draw_build_cmd(struct demo *demo, VkCommandBuffer cmd_buf) {
    ...
    const VkClearValue clear_values[2] = {
        [0] = {.color.float32 = {0.12f, 0.12f, 0.12f, 0.0f}}, // BACKGROUND COLOR
...
void demo_update_data_buffer(struct demo *demo) {
    ...
    mat4x4_rotate_Y(demo->model_matrix, Model, (float)degreesToRadians(demo->spin_state.spin_angle));
...
static void demo_draw(struct demo *demo) {
    ...
    if (demo->VK_GOOGLE_display_timing_enabled) {
        ...
        // Fixed cppCheck invalidLifetime error : Using object that points to local variable 'present_time' that is out of scope.
        // VkPresentTimesInfoGOOGLE present_time = {
...
bool loadTexture(const char* filename, uint8_t* rgba_data, VkSubresourceLayout* layout, int32_t* width, int32_t* height)
{
    (void)filename;
    if (!width || !height) {
        printf("ERROR: invalid params to loadTexture");
        return false;
    }
    *width = NVIDIA_LOGO_width;
    *height = NVIDIA_LOGO_height;
    if (rgba_data == NULL) {
        return true;
    }

    char* cPtr = (char*)NVIDIA_LOGO_rgba;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_MBUTTONUP:
    case WM_RBUTTONUP:
        gfnsdk_handleButtonClick(uMsg, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), &demo.spin_state, demo.width);
        break;
    case WM_KEYDOWN:
        switch (wParam) {
        case VK_ESCAPE:
            ...
            break;
        case VK_LEFT:
            gfnsdk_decreaseSpin(&demo.spin_state);
            break;
        case VK_RIGHT:
            gfnsdk_increaseSpin(&demo.spin_state);
            break;
        case VK_UP:
        case VK_DOWN:
            gfnsdk_reverseSpin(&demo.spin_state);
            break;
        case VK_SPACE:
...
static void demo_create_window(struct demo *demo) {
    ...
    demo->window = CreateWindowEx(0,
        ...
        demo->name,            // app name
        WS_MAXIMIZE | WS_VISIBLE | WS_POPUP, // window style
        100, 100,            // x/y coords
...
static void demo_create_xlib_window(struct demo *demo) {
    ...
    XSelectInput(demo->display, demo->xlib_window, ExposureMask | KeyPressMask | ButtonPressMask | ButtonReleaseMask);
...
static void demo_handle_xlib_event(struct demo *demo, const XEvent *event) {
    ...
    case ClientMessage:
        if ((Atom)event->xclient.data.l[0] == demo->xlib_wm_delete_window)
            demo->spin_state.quit = true;
        break;
    case ButtonRelease:
        gfnsdk_handleButtonClick(event->xbutton.button,
                                event->xbutton.x,
                                event->xbutton.y,
                                &demo->spin_state,
                                demo->width);
...
static void demo_run_xlib(struct demo* demo) {
    while (!demo->spin_state.quit) {
        XEvent event;

        if (demo->spin_state.pause) {
        ...
        demo_draw(demo);
        demo->curFrame++;
        if (demo->frameCount != INT32_MAX && demo->curFrame == demo->frameCount) demo->spin_state.quit = true;
    }
}
#elif defined(VK_USE_PLATFORM_XCB_KHR)
...
static void demo_run(struct demo *demo) {
    ...
    while (!demo->spin_state.quit) {
        while (!screen_get_event(demo->screen_context, demo->screen_event, demo->spin_state.pause ? ~0 : 0)) {
...
static void demo_init(struct demo *demo, int argc, char **argv) {
...
#if defined(ANDROID)
        ERR_EXIT("Usage: CubeSample [--validate]\n", "Usage");
#else
        char *message =
    ...
    demo->spin_state.spin_angle = 1.5f;
    demo->spin_state.spin_increment = 0.2f;
    demo->spin_state.pause = false;
...
int WINAPI WinMain_internal(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow) {
    ...
    demo.connection = hInstance;
    strncpy(demo.name, "Cube Sample", APP_NAME_STR_LEN);
    demo_create_window(&demo);
    demo_init_vk_swapchain(&demo);

    gfnsdk_init(&demo.spin_state);
    demo_prepare(&demo);
...
static void demo_main(struct demo *demo, void *caMetalLayer, int argc, const char *argv[]) {
    ...
    demo_init_vk_swapchain(demo);
    demo_prepare(demo);
    demo->spin_state.spin_angle = 0.4f;
}
